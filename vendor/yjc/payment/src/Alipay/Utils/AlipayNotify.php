...
class AlipayNotify {
	...
	public function decrypt($prestr) {
		return Rsa::rsaDecrypt($prestr, trim($this->alipay_config['private_key_path']));
	}
	...
	public function getSignVeryfy($para_temp, $sign, $isSort) {
		//除去待签名参数数组中的空值和签名参数
		$para = Core::paraFilter($para_temp);
		
		//对待签名参数数组排序
		if($isSort) {
			$para = Core::argSort($para);
		} else {
			$para = $this->sortNotifyPara($para);
		}
		
		//把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
		$prestr = Core::createLinkstring($para);
