...
class AlipaySubmit {
	...
	public function __construct($alipay_config, $alipay_gateway_new = null){
	    $alipay_gateway_new && $this->alipay_gateway_new = $alipay_gateway_new;
	...
	public function buildRequestUrl($para_temp, $method, $button_name) {
	    //待请求参数数组
	    $para = $this->buildRequestParaToString($para_temp);
	    $url = $this->alipay_gateway_new. $para;

	    $sHtml = '<header><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=0"/>
</header>';
