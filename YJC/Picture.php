...
class Picture{
    ...
    private $PICTURE_URL; //要处理的图片
    private $PICTURE_CREATE; //要创建的图片
    private $TRUE_COLOR; //新建一个真彩图象


    private $PICTURE_WIDTH; //原图片宽度
    private $PICTURE_HEIGHT; //原图片高度
     ...
     */
    private $MARK_TYPE = 1;
    private $WORD; //经过UTF-8后的文字
    private $WORD_X; //文字横坐标
    private $WORD_Y; //文字纵坐标
    private $FONT_TYPE; //字体类型
    private $FONT_SIZE = "12"; //字体大小
    private $FONT_WORD; //文字
    private $ANGLE = 0; //文字的角度，默认为0
    private $FONT_COLOR = "#000000"; //文字颜色
    private $FONT_PATH = "font/simkai.ttf"; //字体库，默认为宋体
    private $FORCE_URL; //水印图片
    private $FORCE_X = 0; //水印横坐标
    private $FORCE_Y = 0; //水印纵坐标
    private $FORCE_START_X = 0; //切起水印的图片横坐标
    private $FORCE_START_Y = 0; //切起水印的图片纵坐标


    private $PICTURE_TYPE; //图片类型
    private $PICTURE_MIME; //输出的头部
     ...
     * 裁切，按比例和固定长度、宽度
     */
    private $CUT_TYPE = 1; //裁切类型
    private $CUT_X = 0; //裁切的横坐标
    private $CUT_Y = 0; //裁切的纵坐标
    private $CUT_WIDTH = 100; //裁切的宽度
    private $CUT_HEIGHT = 100; //裁切的高度


    /**
     * 锐化
     */
    private $SHARP = 7.0; //锐化程度


    /**
     * 透明度处理
     */
    private $ALPHA = 100; //透明度在0-127之间
    private $ALPHA_X = 90;
    private $ALPHA_Y = 50;
     ...
     * 任意角度旋转
     */
    private $CIRCUMROTATE = "90.0"; //注意，必须为浮点数
    ...
    private function get_info($PICTURE_URL){
        /**
    ...
    /**
     * 图片缩放
     * 默认是按比例缩放。如果第二个参数为0，只有第一个参数生效
     * @param int $SCALING 代表缩放比例或者缩放宽度
     * @param int $ZOOM_HEIGHT 缩放高度
     * @example
     ...
     * @test ok
     */
    public function zoom($SCALING = 1, $ZOOM_HEIGHT = 0){

        if($ZOOM_HEIGHT == 0){
            $SCALING = $SCALING > 1 ? 1 : $SCALING;
            $this -> ZOOM_WIDTH = $this -> PICTURE_WIDTH * $SCALING;
            $this -> ZOOM_HEIGHT = $this -> PICTURE_HEIGHT * $SCALING;
        }else{
            $this -> ZOOM_WIDTH = $SCALING;
...
// 裁切图片,按坐标或自动
     ...
     * 保存图片
     */
    public function save_picture($DEST_URL = ''){

        if(!$this -> TRUE_COLOR){
            exit($this -> ERROR['unavilable']);
        }
        if (!$DEST_URL){
            exit($this -> ERROR['no_dest_file']);
        }
